...
class CorrectionsMixin:
    ...
    def _sauvegarder_annulation(self):
        self.annuler_stack.append(
            self.donnees.copy()
        )  # On garde une copie du DataFrame actuel
    ...
    def annuler(self):
        ...
        self.donnees = self.annuler_stack.pop()  # On reprend le dernier état sauvegardé
    ...
    def reset_all(self):
        ...
        self.donnees = self.donnees_original.copy()  # Remet à l'état original
        self.sel_x1 = self.sel_x2 = None
        self.click_n = 0
    ...
    def appliquer_correction(self):
        ...
        # Colonnes des bins
        colonnes_bins = [c for c in self.donnees.columns if c.startswith("smps_d_")]
        self.donnees.loc[:, colonnes_bins] *= facteur
        self.donnees.loc[:, "smps_concTotal"] *= facteur
