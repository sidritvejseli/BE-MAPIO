...
class App(tk.Tk, LoaderMixin):
    ...
    # Frame pour les contrôles
        ...
        tk.Button(control_frame, text="Jour précédent", command=self.prev_day_tk).pack(side=tk.LEFT)
        tk.Button(control_frame, text="Jour suivant", command=self.next_day_tk).pack(side=tk.LEFT)
    
        self.plot_day_simple()

    def plot_day_simple(self):
        df_day = self.df[self.df["datetime"].dt.date == self.current_day]
